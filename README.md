...
R2026.03 基线目前追踪 197 个 Capability ID。S01-S13 已完成 Accepted Stage Exit；当前
150 项为 L2、37 项为 L1、10 项为 L0。S13 只确认 Linux A、Container B、native Windows
process/env B（file/network U）及其对应 L2/L1 能力；这不表示已有 Managed Policy、Auto Mode、
全平台同等级 OS Sandbox。S14 working-tree candidate 仍为 IN_PROGRESS、Stage Exit OPEN：12 个注册 seed×5 的 60 个实际 Headless/SDK/stable-v1 场景与独立真实 OpenAI 1 pass/1 conditional skip 已验证；Session canonical control、Plugin global writer recovery/install/uninstall/registry migration、production OTel、stable v1、Provider Router 与机器级 Managed enforcement 已闭环。G4/G5 candidate 证据可复验，但 G6/Stage Exit 必须等待 implementation commit 后的 commit-scoped 验收，不宣称 Accepted 或 L3。
