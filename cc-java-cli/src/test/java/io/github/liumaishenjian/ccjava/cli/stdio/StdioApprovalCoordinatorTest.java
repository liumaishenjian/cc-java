...
class StdioApprovalCoordinatorTest {
    ...
    void matchingAllowOnceReleasesOnlyTheDisplayedRequest() throws Exception {
        ...
        CompletableFuture<PermissionDecision> decision = CompletableFuture.supplyAsync(
                () -> coordinator.requestApproval(invocation(new CancellationSource()), definition()));
        ...
        assertThat(coordinator.resolve("other", PermissionDecision.ALLOW)).isFalse();
        assertThat(decision).isNotDone();

        assertThat(coordinator.resolve("approval-1", PermissionDecision.ALLOW)).isTrue();
        assertThat(decision.get(2, TimeUnit.SECONDS)).isEqualTo(PermissionDecision.ALLOW);
        assertThat(coordinator.resolve("approval-1", PermissionDecision.DENY)).isFalse();
    }
    ...
    void cancellationAndCloseFailClosedAndReleaseWaiters() throws Exception {
        ...
        CompletableFuture<PermissionDecision> cancelled = CompletableFuture.supplyAsync(
        ...
        cancellation.cancel();
        assertThat(cancelled.get(2, TimeUnit.SECONDS)).isEqualTo(PermissionDecision.DENY);
        ...
        CompletableFuture<PermissionDecision> closed = CompletableFuture.supplyAsync(
        ...
        assertThat(closed.get(2, TimeUnit.SECONDS)).isEqualTo(PermissionDecision.DENY);
    }
    ...
    void patchPreviewContainsOnlyRelativeTargetOperationAndLineCounts() throws Exception {
        ...
        CompletableFuture<PermissionDecision> decision = CompletableFuture.supplyAsync(
                () -> coordinator.requestApproval(invocation, definition));
        assertThat(requested.await(2, TimeUnit.SECONDS)).isTrue();

        assertThat(captured.get().preview()).isEqualTo(
                new StdioApprovalCoordinator.Preview(
                        "src/main/App.java", "modify", 2, 3, "", "", ""));
        assertThat(coordinator.resolve("patch-approval", PermissionDecision.DENY)).isTrue();
        assertThat(decision.get(2, TimeUnit.SECONDS)).isEqualTo(PermissionDecision.DENY);
    }
